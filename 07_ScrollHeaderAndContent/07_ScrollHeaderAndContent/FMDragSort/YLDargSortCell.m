...
- (void)cancelSubscribe {
    if (self.delegate && [self.delegate respondsToSelector:@selector(YLDargSortCellCancelSubscribe:)]) {
        [self.delegate YLDargSortCellCancelSubscribe:self.dataModel.title];
    }
}
...
//根据数据源设置UI相关的属性
- (void)setDataModel:(FMDragSortModel *)dataModel {
    _dataModel = dataModel;
    _deleteBtn.hidden = !(dataModel.isTop && [YLDragSortTool shareInstance].isEditing);
    _label.text = dataModel.title;
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]] && ![YLDragSortTool shareInstance].isEditing) || !_dataModel.isTop) {
        return NO;
    }
    return YES;
}
