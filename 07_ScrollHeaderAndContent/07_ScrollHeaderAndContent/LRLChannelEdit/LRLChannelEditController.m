...
#define TitleSize 12.0 //”我的栏目“label字体大小
#define EditTextSize 9.0 //提示编辑label字体大小
...
-(void)configUI{
    ...
    for (int i = 0; i < self.topDataSource.count; ++i) {
        ...
        if (self.locationIndex == i) { //蓝色 //008dff //默认前一个页面选中索引对应的按钮
            touchView.contentLabel.textColor = [UIColor redColor];//UIColorFromRGB(0x008dff);
...
-(void)topTapAct:(UITapGestureRecognizer *)tap{
    ...
    if (_isEditing) {//编辑状态下
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            touchView.closeImageView.hidden = YES;//隐藏右上角的x号
        }];
        //去除拖拽和长按手势
...
-(void)bottomTapAct:(UITapGestureRecognizer *)tap{
    ...
    NSInteger i = self.topViewArr.count - 1;//最后一个位置的索引（也就是即将添加到位置的索引）
    [UIView animateWithDuration:0.3 animations:^{
...
#warning My code
        ...
        if (_moveIndex < self.topViewArr.count && _moveIndex >= 0 ) {//不是拖动到最后位置
            //操作视图
...
#pragma mark - 长按手势
//屏幕的
- (void)mylongTapAct:(UIGestureRecognizer *)gesture {
    _isEditing = YES;
